...
const Sheets = (() => {
  ...
  async function getFixedExpenses() {
    ...
    return rows.map((r,i) => ({
      rowIndex: i+2,
      name:     r[FCOL.NAME]||'',
      amount:   parseFloat(r[FCOL.AMOUNT])||0,
      category: r[FCOL.CAT]||'',
  ...
  async function getInstallments() {
    ...
    return rows.map((r,i) => ({
      rowIndex:  i+2,
      name:      r[ICOL.NAME]||'',
      total:     parseFloat(r[ICOL.TOTAL])||0,
      nRates:    parseInt(r[ICOL.N_RATES])||0,
      paid:      parseInt(r[ICOL.PAID])||0,
      amount:    parseFloat(r[ICOL.AMOUNT])||0,
      category:  r[ICOL.CAT]||'',
      start:     r[ICOL.START]||'',
      note:      r[ICOL.NOTE]||'',
    }));
  ...
  async function getInvestments() {
    ...
    return rows.map((r,i) => ({
      ...
      ticker:    r[INV.TICKER]||'',
      type:      r[INV.TYPE]||'',
      invested:  parseFloat(r[INV.INVESTED])||0,
      current:   parseFloat(r[INV.CURRENT])||0,
      start:     r[INV.START]||'',
      freq:      r[INV.FREQ]||'Manuale',
      periodic:  parseFloat(r[INV.PERIODIC])||0,
  ...
  async function getDeposits(planId) {
    ...
    const all = rows.map((r,i) => ({
      ...
      amount:    parseFloat(r[DCOL.AMOUNT])||0,
