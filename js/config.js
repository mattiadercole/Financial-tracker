// ═══════════════════════════════════════════════
//  FINANCIAL TRACKER — Config & Constants v3
// ═══════════════════════════════════════════════
...
function formatEuro(amount) {
  const n = typeof amount === 'string' ? parseFloat(amount.replace(',','.')) : parseFloat(amount);
  return new Intl.NumberFormat('it-IT',{style:'currency',currency:'EUR',minimumFractionDigits:2}).format(isNaN(n)?0:n);
}

function parseAmount(val) {
  if (typeof val === 'number') return val;
  // Handle both "1.234,56" (IT) and "1234.56" (EN) formats
  ...
  if (lastComma > -1) {
    ...
    if (afterComma.lenght <= 2) return parseFloat(s.replace(',','.'));
    return parseFloat(s.replace(/,/g, ''));
  }
